# Im PPT Generator — 문서 인덱스

| 폴더 | 문서 | 내용 |
|---|---|---|
| **00-overview** | [PROJECT_BRIEF](00-overview/PROJECT_BRIEF.md) | 기획 의도 — 문제정의, 왜/왜/왜, 차별화, SWOT, 성공 기준 |
| | [PRODUCT_REQUIREMENTS](00-overview/PRODUCT_REQUIREMENTS.md) | MoSCoW 기능 요구사항 + 비기능 요구사항 |
| | [USER_SCENARIOS](00-overview/USER_SCENARIOS.md) | 사용자 시나리오 4종 + 유스케이스 |
| | [GLOSSARY](00-overview/GLOSSARY.md) | 용어 사전 |
| **01-research** | [COMPETITIVE_ANALYSIS](01-research/COMPETITIVE_ANALYSIS.md) | 14개 경쟁 제품 기능 매트릭스, 시장 공백 |
| | [TECHNOLOGY_RESEARCH](01-research/TECHNOLOGY_RESEARCH.md) | 렌더링 아키텍처/PPTX/스켈레톤/검색 API 기술 검증 |
| **02-architecture** | [SYSTEM_ARCHITECTURE](02-architecture/SYSTEM_ARCHITECTURE.md) | 시스템 구조도, 데이터 흐름 |
| | [TECH_STACK](02-architecture/TECH_STACK.md) | 스택 선정 + 미선택 이유 |
| | [DATA_MODEL](02-architecture/DATA_MODEL.md) | 도메인 모델, 불변식, 저장 전략 |
| | [TEMPLATE_SYSTEM](02-architecture/TEMPLATE_SYSTEM.md) | 템플릿 3종(빌트인/사용자저장/PPTX추출), 갤러리·저장·선택 흐름 |
| | [ADR-001](02-architecture/ADR-001-json-schema-ssot.md)~[007](02-architecture/ADR-007-api-first-headless.md) | 핵심 결정 (003=GenerationConfig·006=Postgres셀프호스트·007=API-First 헤드리스) |
| **03-api** | [REST_API](03-api/REST_API.md) | API v1 명세(엔드포인트/SSE/에러 코드) |
| **04-design** | [DESIGN_SYSTEM](04-design/DESIGN_SYSTEM.md) | 디자인 토큰(이원 액센트/글래스/타이포) |
| | [UI_UX_GUIDE](04-design/UI_UX_GUIDE.md) | 화면 17종 설계 + 핵심 플로우 + UX 원칙 |
| **05-guides** | [DEV_ENVIRONMENT](05-guides/DEV_ENVIRONMENT.md) | 개발 환경/셋업/env |
| | [CODING_STANDARDS](05-guides/CODING_STANDARDS.md) | 코딩 규칙(기업급 패턴) |
| | [GIT_WORKFLOW](05-guides/GIT_WORKFLOW.md) | 브랜치/커밋/릴리스 |
| | [TESTING_STRATEGY](05-guides/TESTING_STRATEGY.md) | 테스트/QA 전략(실데이터 E2E) |
| **06-devlog** | [ROADMAP](06-devlog/ROADMAP.md) | P0~P10 전체 로드맵(상태 추적) |
| | [DEVELOPMENT_PLAN](06-devlog/DEVELOPMENT_PLAN.md) | 마일스톤 M1~M5 + 진행 규칙 |
| | 날짜별 devlog | phase/세션 단위 개발 기록 |
| 루트 | [CURRENT_STATUS](CURRENT_STATUS.md) | 모듈별 진행률 |
