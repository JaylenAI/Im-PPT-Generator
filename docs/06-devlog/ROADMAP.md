...
- ✅ 검증: Playwright 편집 E2E 2건(인라인 수정·속성 패널)
- ❌ 요소 드래그/리사이즈/회전, 요소 추가 툴바, z-order, 정렬 스냅 — 후속
- ❌ 오버플로 자동 수정(폰트 메트릭) — 후속
...
- ❌ 슬라이드당 디자인 변형 N개 제시→선택

## P9 — 발표 + 출력 확장 🔄 (발표 모드 완료)

- ✅ **발표 모드**: 전체화면 슬라이드 + 키보드 네비(←/→/Space/Esc) + 뷰포트 비율 유지 스케일 + 발표자 노트 토글(N키). Playwright E2E(열기→네비→종료)
- ❌ 발표자 뷰(다음 슬라이드 미리보기), AI 스피커 노트 생성
- ❌ 화면비 4:3/9:16 전 파이프라인 스윕(레이아웃 좌표가 1280×720 하드코딩 — 아키텍처 작업 필요), PNG export(헤드리스 렌더 필요), 웹 링크 퍼블리싱
