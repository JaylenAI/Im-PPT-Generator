...
> 마지막 업데이트: 2026-07-04 · P2~P4 완성 + P5a(WYSIWYG)·P10a(접근성) — 단위 215·타입 16/16·Playwright 3 그린
...
| WYSIWYG 텍스트 인라인 편집 | ✅ 완료 | P5a, Playwright 영속 E2E |
| 접근성 검사(WCAG 대비/alt) | ✅ 완료 | P10a |
| Playwright 브라우저 E2E 하네스 | ✅ 완료 | 게이트·편집 E2E 3건 |
...
- **P5 잔여**: 요소 드래그/리사이즈/회전, 속성 패널, 요소 추가 툴바, undo/redo, 오버플로 자동수정
- **P6**: 템플릿 갤러리 15종+, 브랜드킷(테마 오버라이드), 유저 PPTX 업로드 추출
- **P7**: PDF/DOCX/MD 업로드, 장문 3모드(preserve), URL 임포트, Guide Mode
- **P8**: AI 이미지 생성(키), 컨설팅급 차트(워터폴/Mekko), 다이어그램
- **P9**: 발표 모드+발표자 뷰, 화면비 4:3/9:16 스윕(레이아웃 좌표 아키텍처 필요), PNG export, 웹 퍼블리싱
- **P10 잔여**: 덱 번역, 요약/확장/톤 리라이트, 애널리틱스, MCP 서버
