...
> 마지막 업데이트: 2026-07-04 · **P2~P10 전 phase 커버** — 단위+실PG 233·타입 17/17·Playwright 10 그린 · origin/dev 푸시(`6808b06`)
>
> 남은 것: AI 이미지 생성(외부 키)·PNG export(헤드리스 렌더)·워터폴/Mekko 차트·오버플로 자동수정(폰트 메트릭)·Excel 파싱·15종+ 템플릿/커스텀 저장 — 외부 의존 또는 별도 인프라 필요 항목만.
...
- **P5 잔여**: 요소 드래그/리사이즈/회전, 요소 추가 툴바, 오버플로 자동수정
- **P6**: 템플릿 갤러리 15종+, 브랜드킷(정적 테마 해석→동적 리팩터 필요), 유저 PPTX 업로드 추출
- **P7**: PDF/DOCX/MD 업로드(파서 라이브러리), 장문 3모드(preserve), URL 임포트, Guide Mode
- **P8 잔여**: AI 이미지 생성(키), 컨설팅급 차트(워터폴/Mekko), 추가 다이어그램
- **P9 잔여**: 발표자 뷰, 화면비 4:3/9:16 스윕(레이아웃 좌표 아키텍처 필요), PNG export, 웹 퍼블리싱
- **P10 잔여**: MCP 서버, 조회 애널리틱스, 예상 질문 생성
